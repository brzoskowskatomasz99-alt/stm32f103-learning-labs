...
## 6. T06 两小时稳定性验收（2026-08-15 16:40 完成）

- 起止：14:40 → 16:40，换频 475.5 MHz 后两块板连续挂机 2 小时，期间无人干预。
- 中间抽查（15:24~15:54，30 min 抓取 `2026-08-15_T01_T06_long_COM12.log`）+ 收尾实测（16:40，2 min 抓取 `2026-08-15_T06_final_COM12.log`）：
  - 噪声帧：**0**（换频后全程无外来同频干扰）；
  - 遥测：持续到达（20 s 周期正常，收尾 2 min 内 6 条）；
  - `status publish`：每 60 s 正常发布，收尾值 `{"gw":"node-01","mqtt":true,"link_rate":100,"terminals":[{"dev":"node-02","online":true}]}`；
  - MQTT 长连接 2 小时无断线（无 CONNACK 重连记录）。
- 结论：**通过（真机）**。T06 ✅（link_rate 100 ≥ 95，噪声 0，2 h 稳定）。
