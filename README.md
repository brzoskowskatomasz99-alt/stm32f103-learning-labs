# STM32F103 Learning Labs

STM32F103 课程实验合集。已整理的实验保存在独立目录中，包含可单独打开的 STM32CubeMX 与 Keil MDK 工程；其余历史实验将在找到或恢复可用工程后逐步归档。

## 实验目录

| 编号 | 实验 | 主要内容 | 状态 |
| --- | --- | --- | --- |
| 01 | [I2C OLED](01-i2c-oled/) | I2C1、PB6/PB7、OLED 显示 | 已归档；本次未重新编译或进行开发板验证 |
| 02 | [OLED 小恐龙游戏](02-oled-dino-game/) | OLED 帧缓冲、按键跳跃、碰撞、计分、LED 爆闪 | 已归档；当前构建、下载和开发板行为已验证 |
| 03 | [OLED 文字、图片与 GIF 取模显示](03-oled-font-bitmap-gif/) | UTF-8 中文字模、32×32 图片、GIF 连续帧 | [已归档并提供重建清单](03-oled-font-bitmap-gif/REBUILD-MANIFEST.md)；用户确认构建、下载和开发板显示通过 |
| 04 | [OLED 界面状态机](04-oled-ui-state-machine/) | 环境监测、OLED 多页面、短按/长按、30 秒息屏唤醒、串口与联动控制 | [已归档并提供重建清单](04-oled-ui-state-machine/REBUILD-MANIFEST.md)；用户确认编译、烧录和上板验收通过 |
| 05 | [LLCC68 P2P 双板通信验证](05-llcc68-p2p/) | SPI1、Ra-01SC、TX/RX 条件编译、分阶段接收诊断 | [已归档并提供重建清单](05-llcc68-p2p/REBUILD-MANIFEST.md)；TX 构建、终端下载与 SENT 已验证，网关无 PREAMBLE，P2P 未通过 |

## 历史学习与实操记录

用户确认此前已经完成课程安排的相关学习与实操。多数实验曾在同一个 `Template` 工程中连续修改，未保留每次实验的独立工程快照，因此“以前做过”和“当前可独立复现”分开记录。

| 课程范围 | 主要内容 | 历史完成状态 | GitHub 归档状态 |
| --- | --- | --- | --- |
| 01–04 | STM32 与开发板、软件安装、新建工程、编译烧录、外设与启动流程 | 用户确认做过 | 未保存独立快照 |
| 05–10 | 寄存器与 HAL、GPIO、按键、EXTI、代码封装、SysTick | 用户确认做过 | 未保存独立快照 |
| 11–16 | TIM/PWM、USART、CO2、DHT22、ADC、DMA | 用户确认做过 | 部分代码仍有保留，但没有完整的独立工程快照 |
| 17 | I2C 与 OLED | 用户确认做过 | 已归档为 `01-i2c-oled` |
| 18–19 | LoRa、巴法云与米家生态对接 | 用户确认做过 | 未保存独立快照 |

- 2026-08-03：将当前已完成构建、下载和开发板验收的 OLED 小恐龙游戏归档为 `02-oled-dino-game`。
- 2026-08-04：将用户确认构建、下载和真机显示全部通过的 OLED 文字、图片与 GIF 取模实验归档为 `03-oled-font-bitmap-gif`。
- 2026-08-05：将用户确认编译、烧录和上板验收通过的环境监测 OLED 界面状态机归档为 `04-oled-ui-state-machine`。
- 2026-08-07：将 LLCC68 P2P 双板通信实验归档为 `05-llcc68-p2p`；保留 TX/终端侧通过与网关无 PREAMBLE 的失败证据，不将 SENT 视为无线链路成功。

### 状态说明

- **用户确认做过**：记录学习经历，不等同于当前重新编译或真机验证。
- **已归档**：仓库中存在可检查的源码与工程配置。
- **未保存独立快照**：不要求重新做；以后找到备份或需要复习时，再整理成独立实验目录。

## 使用方法

1. 进入需要学习或复现的实验目录。
2. 使用 STM32CubeMX 打开该目录下的 `.ioc` 文件。
3. 使用 Keil MDK 打开 `MDK-ARM` 下的 `.uvprojx` 工程。
4. 编译、下载并在开发板上验证实验现象。

## 添加新实验

新实验使用连续编号和简短英文名称，例如：

```text
02-gpio-led/
03-timer-pwm/
04-uart/
```

每个实验应保留独立工程，并在自己的 `README.md` 中记录目标、接线、关键代码和验证状态。
